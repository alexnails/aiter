...
def build_dcp_topk_merge_module(
    ...
    def select_kernel(
        ...
        # Not collapsible into one `if`: the outer test is a compile-time
        # constant that removes the store entirely, the inner one is a runtime
        # thread predicate. Merging them would make the whole thing runtime.
        if const_expr(write_threshold):  # noqa: SIM102
            if tid == 0:
                debug_threshold[row] = s_acc[0]
